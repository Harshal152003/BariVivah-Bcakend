  ...
  try {
    ...
    if (interestId) {
      ...
      return NextResponse.json(
        { success: false, message: "Either interestId or (senderId and targetUserId) is required" },
    ...
    if (!targetInterest) {
      return NextResponse.json(
        { success: false, message: "Interest request not found" },
    ...
    if (targetInterest.status !== 'pending') {
      return NextResponse.json(
        { success: false, message: `Cannot withdraw request with status '${targetInterest.status}'. Only pending requests can be withdrawn.` },
        { status: 400, headers: corsHeaders }
      );
    }
    ...
    console.error("Error in POST withdraw interest:", error);
    return NextResponse.json(
